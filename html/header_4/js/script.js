		...
		Form: function() {
			...
			$('.affa-form-subscribe').submit(function() {
				...
				if (pattern.test($email.val()) === false) {
					$email.val('Please enter a valid email address!').removeClass('success').addClass('error');
				} else {
					...
					$.ajax({
						...
						success: function(msg) {
							if (parseInt(msg, 0) !== 0) {
								...
								if (msg_split[0] === 'success') {
									$submit.removeAttr('disabled');
									$email.removeAttr('disabled').val(msg_split[1]).removeClass('error').addClass('success');
								} else {
									$submit.removeAttr('disabled');
									$email.removeAttr('disabled').val(msg_split[1]).removeClass('success').addClass('error');
								}
							}
						}
					});
